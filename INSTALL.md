...
S-AIProviders 提供 4 种使用方式，按场景挑一种即可。
...
> 第一次跑会下载 `@s-aiproviders/cli` 到 npm 缓存（约 20 KB + 依赖 core 约 25 KB），后续直接命中缓存。
...
# 或 pnpm add -g @s-aiproviders/cli
...
完整 schema 见 `node_modules/@s-aiproviders/cli/references/config/extend-md-schema.md`。
...
`node_modules/@s-aiproviders/cli/references/integration-as-library.md` 或
本仓库 `skill/references/integration-as-library.md`。

---

## 方式 4：作为 AI Agent Skill（让 AI 自动调用）

S-AIProviders 同时是一份 [Anthropic Skill](https://docs.claude.com/en/docs/claude-code/skills) 兼容的能力包。装包后，`SKILL.md` 就在 `node_modules/@s-aiproviders/cli/SKILL.md`，AI agent 读到 frontmatter 里的 `description` 就会在适当时候自动调用 CLI。

### 4-A：Claude Code / Cursor / 类 Agent 自动加载

把 SKILL.md + 脚本拷到 agent 约定的 skills 目录。Claude Code 的位置：
...
让 AI 助手读一下：
```
请阅读 https://unpkg.com/@s-aiproviders/cli/SKILL.md，
...
5. **Preset 内置默认** — base URL 和首个 builtin model
...
cd skill         && pnpm build && npm publish --access public
```

`prepublishOnly` 钩子已配置，会在 publish 前自动 clean+build。`workspace:*` 在 pnpm pack 时自动转换为版本号。
